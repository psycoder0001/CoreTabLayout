...
public class CoreTabLayout extends LinearLayout {
    ...
    private boolean useTextViewView = false;
    ...
    private void generateItemViewWith(CoreTabItem coreTabItem, boolean isPassive) {
        TextView itemView = useTextViewView ? new Button(getContext()) : new TextView(getContext());
    ...
    private void selectItem(CoreTabItem tabItem) {
        ...
        if (tabItem.selectedItemTv.getAlpha() == 1f && tabItem.selectedItemTv.getVisibility() == VISIBLE) {
            // Already selected.
            return;
        }
